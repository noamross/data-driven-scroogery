...
I had to go shopping for a family birthday and new apartment stuff this. I
...
<blockquote class="twitter-tweet" data-lang="en"><p lang="en" dir="ltr">It begins. <a href="https://t.co/sIqgGZcKfF">pic.twitter.com/sIqgGZcKfF</a></p>&mdash; Zach Cole 🌈 (@ZachACole) <a href="https://twitter.com/ZachACole/status/793920311414534144?ref_src=twsrc%5Etfw">November 2, 2016</a></blockquote>
...
the rush of the waters of a gigantic rapid before I see it and it eventually swallows and flips the boat. [^1]

[^1]: I want to emphasize this is no way a judgement on Mariah Carey or, actually,
the song itself.  But like "Free Bird," it's now impossible to
separate it from its place in our culture as, in this case, the death knell of normal 
human relations for the year.

Worried about what the early arrival of the song portended, I sought out an
update to Zach Cole's observation.  It turns out, while YouTube's new layout
hides the "Statistics" button, if you go to the old layout (`youtube.com/old/...`),
you can still find the data  In fact, the raw data is in a hidden table in the page
...
cc <- cc %>%
    mutate(prediction = as.integer(exp(predict(expmod, newdata=cc)))) %>% 
...
Based on this model, views will grow ~49% every year and the song views will
exceed human population in another 12 years.

As an ecologist, though, I can thankfully point to a body of theory showing exponential
...
will all collapse into starvation, cannibalism or a lucky supervolcano eruption. 
...
and see what weekend's shopping experience tells us about what's to come.
...
```{r}
cc <- cc %>%
    mutate(pred2 = as.integer(10^predict(lmod, newdata=cc)),
...
We get almost the same result! Though maybe, just maybe, this force of nature
has lost some momentum this year. 
